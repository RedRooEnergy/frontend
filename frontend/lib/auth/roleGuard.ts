...
type Role = "buyer" | "supplier" | "regulator" | "freight";
...
export function requireBuyer(headers: Headers): RoleContext | null {
  const dev = devRole(headers, "buyer", "x-dev-buyer");
  if (dev) return dev;
  const cookieHeader = headers.get("cookie");
  const session = getSessionFromCookieHeader(cookieHeader);
  if (!session || session.role !== "buyer") return null;
  return { actorId: session.userId || session.email, actorRole: "buyer" };
}
