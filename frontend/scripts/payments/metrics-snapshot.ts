...
function parseCliOptions(argv: string[]): CliOptions {
  ...
  return {
    ...
    secret: parseArgValue(argv, "--secret") || process.env.PAYMENTS_METRICS_JOB_SECRET,
...
async function runLocal(options: CliOptions) {
  return runPaymentsMetricsSnapshot({
...
async function runHttp(options: CliOptions) {
  ...
  if (!url) {
    throw new Error("metrics snapshot cli requires --url for http mode");
  }

  const secret = String(options.secret || "").trim();
  if (!secret) {
    throw new Error("metrics snapshot cli requires --secret or PAYMENTS_METRICS_JOB_SECRET for http mode");
  }
  ...
  if (!response.ok) {
    throw new Error(`metrics snapshot http failed (${response.status}): ${text}`);
  }

  try {
    return JSON.parse(text);
  } catch {
    throw new Error("metrics snapshot http returned non-json response");
  }
}
