...
import { activateAuthorityEnforcementKillSwitch } from "../../lib/governance/authority/enforcementControlStore";
...
function parseCliOptions(argv: string[]): CliOptions {
  ...
  return {
    ...
    triggerBy: parseArgValue(argv, "--triggeredBy") || "authority-enforcement-auto-guard.ts",
    thresholds: {
...
async function runHttp(options: CliOptions) {
  ...
  if (!url) {
    throw new Error("authority auto-guard requires --url for http mode");
  }

  const secret = String(options.secret || "").trim();
  if (!secret) {
    throw new Error("authority auto-guard requires --secret or GOV04_AUTHORITY_SHADOW_METRICS_JOB_SECRET");
  }
  ...
  if (!response.ok) {
    throw new Error(`authority auto-guard http failed (${response.status}): ${text}`);
  }

  try {
    return JSON.parse(text) as AuthorityShadowMetricsReport;
  } catch {
    throw new Error("authority auto-guard http returned non-json response");
  }
}
...
async function main() {
  ...
  const guard = evaluateAuthorityEnforcementGuard(report, options.thresholds);
  const persistedGuard = await appendAuthorityEnforcementGuardReport({
    report,
    guard,
    metadata: {
      script: "authority-enforcement-auto-guard.ts",
      mode: options.mode,
      autoActionEvaluated: true,
    },
  });

  let killSwitchActivation:
    | {
        activated: false;
        controlEventId: null;
      }
  ...
  if (guard.rollbackRecommended) {
    const activated = await activateAuthorityEnforcementKillSwitch({
      reasonCode: "GUARD_PAGE",
      guardReportId: persistedGuard.record.guardReportId,
      reportHashSha256: guard.reportHashSha256,
      triggeredBy: options.triggerBy,
      eventAtUtc: guard.evaluatedAtUtc,
      metadata: {
        script: "authority-enforcement-auto-guard.ts",
        guardOverallStatus: guard.overallStatus,
        killSwitchAction: guard.killSwitchAction,
      },
    });
    killSwitchActivation = {
      activated: true,
      controlEventId: activated.record.controlEventId,
  ...
  const output = JSON.stringify(
    {
      ...
      persistence: {
        enabled: true,
        created: persistedGuard.created,
        guardReportId: persistedGuard.record.guardReportId,
      },
      killSwitchActivation,
    },
