...
import { getOrders, writeStore } from "../../../../../lib/store";
...
export async function POST(request: Request) {
  ...
  orders[idx] = {
    ...
    timeline: [
      ...(orders[idx].timeline || []),
      { status: "DELIVERED", timestamp: now, note: "Order completed by admin" },
    ],
  };
  ...
  dispatchFreightAuditLifecycleHook({
    source: "api.admin.orders.complete",
    ...
    context: {
      ...
      deliveredAt: orders[idx].deliveredAt || null,
      availableEvidenceCodes: ["DELIVERY_ATTEMPT_LOG"],
    },
  });
  ...
  try {
    ...
    console.error("Order completed buyer email failed", error);
  }

  // Supplier ORDER_COMPLETED
  ...
  for (const supplierId of supplierIds) {
    ...
    try {
      ...
      console.error("Order completed supplier email failed", error);
    }
  }
