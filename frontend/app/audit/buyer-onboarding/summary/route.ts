...
function resolveRepoRoot() {
  ...
  for (const candidate of candidates) {
    if (fs.existsSync(path.join(candidate, "artefacts", "buyer-onboarding-audit"))) {
      return candidate;
    }
  }
...
<head>
  ...
  <title>Buyer Onboarding Audit Summary</title>
  <style>
...
<body>
  <div class="wrap">
    <h1>Buyer Onboarding Audit Summary</h1>
    <div class="meta">
...
export async function GET(request: Request) {
  ...
  const scorecardPath = path.join(rootDir, "artefacts", "buyer-onboarding-audit", `scorecard.buyer-onboarding.${runId}.json`);
