...
function resolveRepoRoot() {
  ...
  for (const candidate of candidates) {
    if (fs.existsSync(path.join(candidate, "artefacts", "freight-customs-audit"))) {
      return candidate;
    }
  }
...
<head>
  ...
  <title>Freight & Customs Audit Summary</title>
  <style>
...
<body>
  <div class="wrap">
    <h1>Freight & Customs Audit Summary</h1>
    <div class="meta">
...
export async function GET(request: Request) {
  ...
  const scorecardPath = path.join(rootDir, "artefacts", "freight-customs-audit", `scorecard.freight-customs.${runId}.json`);
