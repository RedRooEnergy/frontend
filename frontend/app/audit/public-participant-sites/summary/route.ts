...
function resolveRepoRoot() {
  ...
  for (const candidate of candidates) {
    if (fs.existsSync(path.join(candidate, "artefacts", "public-participant-sites"))) {
      return candidate;
    }
  }
...
function renderHtml(scorecard: any, sha256Value: string) {
  ...
  const orderedChecks = checks.slice().sort((a, b) => String(a?.id ?? "").localeCompare(String(b?.id ?? "")));
  ...
  const rows = orderedChecks
    .map((check) => {
      ...
      const details = escapeHtml(detailsRaw.length > 260 ? `${detailsRaw.slice(0, 260)}...` : detailsRaw);
...
<head>
  ...
  <title>Public Participant Sites E2E Audit Summary</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .meta { margin-top: 12px; background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 12px; font-size: 12px; }
    ...
    .grid { margin-top: 16px; display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px; }
    ...
    @media print {
      ...
      .card, table, .outputs, .meta, .overall { border-color: #ddd; }
...
<body>
  <div class="wrap">
    <h1>Public Participant Sites E2E Audit Summary</h1>
    <div class="meta">
    ...
    <div class="grid">
      <div class="card"><div class="k">Total</div><div class="v">${escapeHtml(String(summary.totalChecks ?? 0))}</div></div>
      ...
      <div class="card"><div class="k">NOT_APPLICABLE</div><div class="v">${escapeHtml(String(summary.notApplicableCount ?? 0))}</div></div>
    </div>
    ...
    <div class="footer">PASS-2 results are non-blocking and not included in overall status.</div>
  </div>
</body>
</html>`;
}
...
export async function GET(request: Request) {
  ...
  const primaryScorecardPath = path.join(
    rootDir,
    "artefacts",
    "public-participant-sites",
    `scorecard.${runId}.json`
  );
  const legacyScorecardPath = path.join(
    rootDir,
    "artefacts",
    "public-participant-sites-audit",
    `scorecard.public-participant-sites.${runId}.json`
  );
  const scorecardPath = fs.existsSync(primaryScorecardPath) ? primaryScorecardPath : legacyScorecardPath;
