...
import { ext05Enabled } from "../../../../lib/featureFlags";
import { addExtensionRecord } from "../../../../lib/store";
import { recordAudit } from "../../../../lib/audit";

export default function Ext05Page() {
  const enabled = ext05Enabled();
  ...
  const submit = () => {
    ...
    recordAudit("EXT05_ACTION_INITIATED", { title });
    addExtensionRecord("ext05Records", {
      id: crypto.randomUUID(),
      title,
      description,
      createdAt: new Date().toISOString(),
    });
    recordAudit("EXT05_ACTION_COMPLETED", { title });
  ...
  return (
    <div className="min-h-screen bg-surface-muted text-strong">
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-4">
        <h1 className="text-2xl font-bold">EXT-05 (Wave 1)</h1>
        {!enabled && (
