...
import { ext02Enabled } from "../../../../lib/featureFlags";
import { addExtensionRecord } from "../../../../lib/store";
import { recordAudit } from "../../../../lib/audit";

export default function Ext02Page() {
  const enabled = ext02Enabled();
  ...
  const submit = () => {
    ...
    recordAudit("EXT02_ACTION_INITIATED", { title });
    addExtensionRecord("ext02Records", {
      id: crypto.randomUUID(),
      title,
      description,
      createdAt: new Date().toISOString(),
    });
    recordAudit("EXT02_ACTION_COMPLETED", { title });
  ...
  return (
    <div className="min-h-screen bg-surface-muted text-strong">
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-4">
        <h1 className="text-2xl font-bold">EXT-02 (Wave 1)</h1>
        {!enabled && (
