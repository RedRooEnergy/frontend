...
type FxPolicyFormProps = {
  initialRules: Record<string, unknown> | null;
  onReview: (draft: Record<string, unknown>) => void;
  disabled?: boolean;
};

export default function FxPolicyForm({ initialRules, onReview, disabled = false }: FxPolicyFormProps) {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <h3 className="text-base font-semibold text-slate-900">FX Policy Draft</h3>
      <p className="mt-1 text-xs text-slate-600">Define updated FX policy rules as JSON and review before submitting.</p>
      <textarea
        value={value}
