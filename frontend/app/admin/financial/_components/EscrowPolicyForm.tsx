...
type EscrowPolicyFormProps = {
  initialRules: Record<string, unknown> | null;
  onReview: (draft: Record<string, unknown>) => void;
  disabled?: boolean;
};

export default function EscrowPolicyForm({ initialRules, onReview, disabled = false }: EscrowPolicyFormProps) {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <h3 className="text-base font-semibold text-slate-900">Escrow Policy Draft</h3>
      <p className="mt-1 text-xs text-slate-600">Core escrow invariants remain non-bypassable and enforced server-side.</p>
      <textarea
        value={value}
